...
impl Evaluator {
    ...
    pub fn evaluate(&mut self, statements: Vec<Stmt>) -> Result<Vec<String>, EvaluatorError> {
    ...
    fn evaluate_expr(&mut self, expr: &Expr) -> Result<Value, EvaluatorError> {
...
impl expr::ExprVisitor<ExprResult> for Evaluator {
    ...
    fn visit_grouping_expr(&mut self, expr: &Expr) -> ExprResult {
        match expr {
            Expr::Grouping { expression } => return self.evaluate_expr(expression),
            _ => return Err(EvaluatorError::DifferentExpression {
                expr: expr.clone(),
    ...
    fn visit_list_expr(&mut self, expr: &Expr) -> ExprResult {
        match expr {
            Expr::List { items } => {
                ...
                for item in items {
                    list.push(self.evaluate_expr(item)?);
                }
        ...
        match expr {
            Expr::Splice { list, is_splice, start, end } => {
                ...
                if let Some(start) = start {
                    start_idx_expr = Some(self.evaluate_expr(start)?);
                }
                if let Some(end) = end {
                    end_idx_expr = Some(self.evaluate_expr(end)?);
                }
    ...
    fn visit_for_stmt(&mut self, stmt: &Stmt) -> StmtResult {
        match stmt {
            Stmt::For { initializer, condition, increment, body } => {
                ...
                let mut condition_evaluation = match self.evaluate_expr(condition) {
                    Ok(v) => v,
                    Err(e) => return Err(Err(e)),
                };
                ...
                while condition_result {
                    ...
                    if increment.is_some() {
                        let _ = match self.evaluate_expr(increment.as_ref().unwrap()) {
                            Ok(v) => v,
                            Err(e) => return Err(Err(e)),
                        };
                    }
                    condition_evaluation = match self.evaluate_expr(condition) {
                        Ok(v) => v,
                        Err(e) => return Err(Err(e)),
                    };
    ...
    fn visit_if_stmt(&mut self, stmt: &Stmt) -> StmtResult {
        match stmt {
            Stmt::If { condition, then_branch, else_branch } => {
                let condition_evaluation = match self.evaluate_expr(condition) {
                    Ok(v) => v,
                    Err(e) => return Err(Err(e)),
                };
    ...
    fn visit_print_stmt(&mut self, stmt: &Stmt) -> StmtResult {
        match stmt {
            Stmt::Print { expression } => {
                let value = match self.evaluate_expr(expression) {
                    Ok(v) => v,
                    Err(e) => return Err(Err(e)),
                };
                match value {
    ...
    fn visit_return_stmt(&mut self, stmt: &Stmt) -> StmtResult {
        match stmt {
            Stmt::Return { keyword: _, value } => {
                ...
                if value.is_some() {
                    return_value = match self.evaluate_expr(value.as_ref().unwrap()) {
                        Ok(v) => v,
                        Err(e) => return Err(Err(e)),
                    };
                }
        ...
        match stmt {
            Stmt::Var { name, initializer } => {
                ...
                if let Some(initializer_expr) = initializer {
                    value = match self.evaluate_expr(initializer_expr) {
                        Ok(v) => v,
                        Err(e) => return Err(Err(e)),
                    };
                }
